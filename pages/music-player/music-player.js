...
const isLoginPlayer = App.globalData.isLoginPlayer
Page({
    data:{
        //滚动视口高度
        scrollHeight:500,
    ...
    onLoad(options){
        ...
        const id=options.id
        const activeNumber = options.current;
        if(activeNumber) this.setData({activeNumber})
        ...
        this.playStoreBindings=createStoreBindings(this,{
                ...
                "circulateNumber",
                "circulateImg"],
                actions:['playMediaqRequest','setPlayList','setsliderChange','setsliderChanging','setbindPlayBack','switchMusic','circulateImgSwitch','setdetermineSwitch','tapLyricToPalyTime']
        })
        wx.nextTick(()=>{
            if(id !== this.id){
                this.playMediaqRequest(id)
            }
        })
        },
        goback(){
        ...
        wx.nextTick(()=>{
            this.setPlayList(data,playListIndex)
        })
        },  
        onUnload(){
            // audioContext.pause()
        ...
        onTapCirculateImg(){
            this.circulateImgSwitch()
        },
        determineSwitch(e){
            const type = e.mark.type;
            this.setdetermineSwitch(type)
        },
        onTapLyricToPalyTime(e){
            const time = e.mark.item.time
            this.tapLyricToPalyTime(time)
        }
